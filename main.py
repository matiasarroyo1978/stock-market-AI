        ...
        with create_account_tab:
            ...
            if st.button("Create Account"):
                ...
                if email_validation is not True:
                    st.caption(email_validation)
                    return
                if password_validation is not True:
                    st.caption(password_validation)
                    return
    ...
    if st.session_state.user_authenticated:
        with st.sidebar:
            with st.expander("Settings",expanded=True):
                asset_tickers = sorted(['DOW','NVDA','TSL','BRK-B','BRK-A','GOOGL','AMZN','AI','NIO','LCID','F','LYFY','AAPL', 'MSFT', 'BTC-USD', 'ETH-USD'])
